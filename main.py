...
async def on_message(message):
    ...
    if message.content.casefold().startswith("mother."):
        ...
        if any(word in message.content.casefold()
               for word in ["hello", "hi", "hey", "wasup"]):
        ...
        if msgrest.startswith(("list.özlüsöz", "list.vdq")):
            ...
            index = 0
            for vdq in vdquotes:
                vdqtext, vdqauthor_name, vdqdate, vdqattachment = vdq[0], vdq[
                    1], vdq[3], vdq[4]
                vdqdate = datetime.strptime(vdqdate, '%Y-%m-%d %H:%M:%S.%f')
                response = response + " ||Index: {0}||".format(
                    index
                ) + '     "{0}"'.format(vdqtext) + " {1}  - {0}, ".format(
                    vdqauthor_name,
                    vdqattachment) + '{0}'.format(str(vdqdate.year) + '\n\n')
                index += 1
        ...
        if db["responding.verydeepquotes"]:
            ...
            if "yaz bunu" in message.content.casefold() or msgrest.startswith(
                ("newvdq ", " newvdq ")):
                msg_obj = message
                ...
                try:
                    ...
                    if message.reference is not None:
                        ...
                        new_vdqtext = re.split('yaz bunu ',
                                               message.content,
                                               flags=re.IGNORECASE)[1]

                fresponse, fresponse_attachmenturl = vdq_f.update_verydeepquotes(
                    new_vdqtext, msg_obj)

                if fresponse_attachmenturl != "":
                    await message.channel.send(fresponse_attachmenturl,
            ...
            if any(word in message.content.casefold()
                   for word in ("özlü söz", "vdq", "very deep", "deep quote",
                                "deep wisdom", "laf", "özlüsöz")):
                if "verydeepquotes" in db.keys():
                    vdquotes = db["verydeepquotes"]
                    ...
                    if randi > len(vdquotes) - 1:
                        ...
                        return
                    vdq = vdquotes[randi]
                    vdqtext, vdqauthor_name, vdqdate, vdqattachment = vdq[
                        0], vdq[1], vdq[3], vdq[4]
            ...
            if msgrest.casefold().startswith(
                ("sil", " sil", "del", " del", "delete", " delete")):
                vdquotes = []
                if "verydeepquotes" in db.keys():
                    ...
                    fresponse = vdq_f.delete_verydeepquotes(
                        index, message.author)
                    print(fresponse)
                    await message.channel.send(fresponse,
                                               allowed_mentions=no_mentions)
                return
        ...
        if any(word in message.content.casefold()
               for word in ["wisdom", "quote"]):
            quote = get_quote2()
            await message.channel.send(quote)
            return

        if any(word in message.content.casefold()
               for word in ("Thanks", "thank you", "appreciated", "thx",
                            "thank")):
        ...
        for w in list_of_words:
            ...
            try:
                ...
                response += random.choice(
                    words.weather_responses1) + " " + weather_info[
                        0] + " and weather is " + weather_info[1].lower(
